"""Тест плейбука: Mistral-24B + gold-документ + ГОТОВЫЕ образцовые ответы -> адаптируй."""
    ...
    for i, d in enumerate(angry):
        ...
        res = gen._llm.create_chat_completion(
            messages=[{"role": "system", "content": SYS + nothink},
                      {"role": "user", "content": build_user(doc_text, ans, d["transcript"])}],
            max_tokens=220, temperature=0.4, top_p=0.9,
            stop=["\n\n\n", "<|im_end|>", "<|message_sep|>"],
        )
        out.append({"dialogue_id": d["dialogue_id"], "turn_idx": d["turn_idx"],
                    "asr_text": d["asr_text"], "ideal_text": d["ideal_text"],
                    "raw": clean(res["choices"][0]["message"]["content"])})
        if (i + 1) % 25 == 0:
            print(f"  {i+1}/{len(angry)}", flush=True)
    (R / "bench_playbook.json").write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\n{len(out)} -> bench_playbook.json")

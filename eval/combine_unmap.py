"""Развернуть судью связки."""
import json
from collections import Counter
from pathlib import Path

R = Path(__file__).resolve().parents[1] / "eval" / "results"
ver = json.loads((R / "combine_judge.json").read_text(encoding="utf-8"))["verdicts"]
mp = {(m["dialogue_id"], m["turn_idx"]): m
      for m in json.loads((R / "combine_map.json").read_text(encoding="utf-8"))}
agg = {}
for v in ver:
...
print("=== Связка (Mistral+MoE) vs Mistral-24B vs MoE (87 злых, слепой судья) ===\n")
for lbl in ["Связка", "Mistral-24B", "MoE"]:
